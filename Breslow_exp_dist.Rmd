---
title: "Analysis of Breslow's estimator using exponential distribution"
...
for(m1 in 1:M){
  for (n1 in 1:n){
    ...
    random_uniform[n1,m1] = runif(1,0,1)
    
    # find the infection time using the probability integral transform 
    infection_time[n1,m1] = log((exp(r*min(data$E[n1],data$S[n1])) - exp(r*data$B[n1])) * random_uniform[n1,m1] + exp(r*data$B[n1])) / r
...
plot(matrix_grid, survival_prob_female, xlab = "Time (days)", ylab = " Survival probability", type = "s" , main = "Estimated survival function for incubation period based on gender", col = "red")
...
plot(matrix_grid, survival_prob_young, xlab = "Time (days)", ylab = "Survival Probability", type = "s" , main = "Estimated survival function for incubation period based on age (old >46)", col = "red")
